<template>
    <v-form v-model="valid">
        <v-container>
            <v-card >
                <v-subheader class="title"> <v-icon large color="blue darken-2">mdi-label</v-icon>UPDATE ITEM
                </v-subheader>
                <form>
                    <v-row>
                        <v-col
                        ...
                        >
                            ...
                            <v-divider></v-divider>
                            <SelectGroup v-model="group"  :group="group"  @selected="groupSelected" :clear="clear" :disabled="create_group"></SelectGroup>
                            <v-checkbox
                    ...
                    <v-btn class="error" @click="submit">cancel</v-btn>
        ...
        methods:{
            ...
            renderData(){
                this.$http.get("/inventory/item/"+ this.$route.params.id).then(response => {
                    ...
                    this.setItemWeight(response.data.data.item_weight.toFixed(2))
                    this.uomItemSelected(response.data.data.item_uom)
                    this.categorySelected(response.data.data.category_type)
                    this.groupSelected(response.data.data.group)
                });
            },
            setItemWeight(price) {
                this.form.item_weight = price;
            },
        },
        mounted(){
            this.renderData()

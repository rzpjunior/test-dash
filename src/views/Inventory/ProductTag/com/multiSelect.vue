...
<script>
    export default {
        ...
        props: ['product_tag','disabled','clear','label','error','aux_data', 'norequired'],
        methods: {
            ...
            autoSelectByID(val) {
                if(val){
                    ...
                    this.$http.get("/inventory/tag_product"+ val.id).then(response => {
                        ...
                        this.product_tag = response.data.data
            ...
            remove (item) {
                const index = this.product_tag.indexOf(item.name)
                if (index >= 0) this.product_tag.splice(index, 1)
            },
        },
        mounted() {
